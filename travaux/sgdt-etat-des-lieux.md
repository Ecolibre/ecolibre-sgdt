...
**Tout est traité au 28 juillet.** Les info-bulles fonctionnent : le mécanisme
est la fonction d'analyse `{{#info: texte}}` de Semantic MediaWiki, placée dans
la cellule du libellé juste après celui-ci — et non un paramètre de la balise
`field`, où ni `info=` ni `description=` ne sont valides (seul `placeholder=`
affiche quelque chose, à l'intérieur du champ). Le rendu est produit par le
serveur, donc vérifiable par `curl`.

La colonne « wiki faisant autorité » a été ajoutée au registre. **Aucun wiki
partenaire n'existe encore** : `add.ecolibre.org` et `cwl.ecolibre.org` y
figurent comme domaines prévus, pas comme wikis en service. L'autorité est donc
`wiki.ecolibre.org` pour les trois codes aujourd'hui — la cellule d'ADD reste à
renseigner, laissée vide faute d'instruction au moment de l'écriture.

**Conséquence à garder en tête :** toute l'architecture fédérée décrite dans ce
document est prospective. La ferme ne sert qu'un seul wiki SGDT à ce jour, et la
colonne d'autorité ne prendra son sens que le jour où un partenaire aura le
sien — elle sert à marquer les exceptions, pas à recopier la colonne
précédente.
